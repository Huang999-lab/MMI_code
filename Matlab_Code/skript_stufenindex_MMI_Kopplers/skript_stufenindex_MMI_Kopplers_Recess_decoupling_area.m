...
L_x = 210e-6; % Länge der querschnitt in x
L_y = 110e-6;
n_glass = 1.522;
n_1 = 1.530; % 1.530
neff = (n_glass+n_1)/2;
...
% Grid
x = linspace(-L_x/2,L_x/2,211);    
y = linspace(-L_y/2,L_y/2,111);
...
L_recess = 500e-6;
z_recess = [z_decoup(1):dz:z_decoup(1) + L_recess];

rad_Recess_button = 50e-6;
rad_decoup_top = 25e-6;
...
a = repmat(n,[1 1 length(z_recess)]);
nc = cat(3,nc,a);


% button output
L_recess_out = 500e-6;
z_recess_out = [z_recess(end) + dz:dz:z(end)];
rad_decoup_top = 25e-6;
rad_decoup_butten = 25e-6;

n = ones(length(y),length(x))*n_glass; 
B=sqrt((xg(:,:,1)-top_center(2)).^2+(yg(:,:,1)-top_center(1)).^2)<=rad_decoup_top;
n(B)=n_1;
B=sqrt((xg(:,:,1)-butten_center(2)).^2+(yg(:,:,1)-butten_center(1)).^2)<=rad_decoup_butten;
n(B)=n_1;

a = repmat(n,[1 1 length(z_recess_out)]);
nc = cat(3,nc,a);
...
surf(z(2:end)*1e6,x*1e6,squeeze(n_int(:,2:end))/max(max(n_int(:,2:end))))
xlabel('z [\mum]')
ylabel('x [\mum]')
colorbar;
shading flat
