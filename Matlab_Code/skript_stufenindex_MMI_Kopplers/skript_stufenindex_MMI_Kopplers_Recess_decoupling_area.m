%% Recess in the decoupling area/Aussparung im Bereich der Ausgangswellenleiter
...
n = ones(length(y),length(x))*n_glass; 
z_decoup = [150*dz:dz:199*dz];% z_decoup = z_recess + z_button_output
...
n = ones(length(y),length(x))*n_glass; 
rad_Recess_button = 20*dx_coarse;
z_recess = [150*dz:dz:179*dz];
B=sqrt((xg(:,:,1)-center_top_x).^2+(yg(:,:,1)-center_top_y).^2)<=rad_decoup_top;
n(B)=n_1;
B=sqrt((xg(:,:,1)-center_button_x).^2+(yg(:,:,1)-center_button_y).^2)<=rad_Recess_button;
n(B)=n_1;
...
% button output
n = ones(length(y),length(x))*n_glass; 
z_button_output = [180*dz:dz:199*dz];
B=sqrt((xg(:,:,1)-center_top_x).^2+(yg(:,:,1)-center_top_y).^2)<=rad_decoup_top;
n(B)=n_1;
B=sqrt((xg(:,:,1)-center_button_x).^2+(yg(:,:,1)-center_button_y).^2)<=rad_decoup_button;
n(B)=n_1;

n = repmat(n,[1 1 length(z_button_output)]);
nc = cat(3,nc,n);

figure
surf(xg(:,:,1),yg(:,:,1),nc(:,:,151))
xlabel('x')
ylabel('y')
zlabel('n')
title('Recess cross section (slice 151-180)')
...
ylabel('y')
zlabel('n')
title('Button output cross section (slice 181-199)')
%% x-z Visualization
figure
...
ylabel('x [\mum]')
zlabel('n')
title('Recess decoupling: x-z cross section after y-integration')
