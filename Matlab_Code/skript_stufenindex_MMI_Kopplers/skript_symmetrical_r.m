%% Exemplary BPM calculation for a rectangular step-index MMI-element Kopplers/ Iterate over r(Radius from Input) in x-y Cross section
...
L_MMI = 2000e-6; % Length for MMI area
r_coup = linspace(5e-6,40e-6,8); % Radius for coupling area

for i = 1:length(r_coup)
    z = [0:dz:L_MMI];                             % Base vector
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % coupling Input area
    Excitation.gausscoup=sqrt(xg(:,:,1).^2+yg(:,:,1).^2)<=r_coup(i);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    file_name = [num2str(r_coup(i)) '_r_coup' '.mat']; % 让保存的文件名随着参数的改变而改变
    save(file_name,'Ex_int','-v7.3')

end

%% save Var and animation
save('r_coup.mat','r_coup','-v7.3')

for i = 1:length(r_coup)
    r_coupstep=r_coup(i)*1e6;
    Ex_int = load(['F:\studium\Master_arbeit\FD\', num2str(r_coup(i)), '_r_coup', '.mat']);
    imagesc(Ex_int.Ex_int);
    colorbar;
    axis xy;
    title(['E_x, r_{coup} = ',num2str(r_coupstep), 'um']);
    xlabel('x coordinate');
    ylabel('y coordinate');
    pause(1)
end
